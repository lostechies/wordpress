...
?>
	...
	function convert_image_paths($post_content, $post_author)
	{
		$user_paths = array(
		  "2118" => array("chad_myers","chadmyers"),
		  "2131" => array("chrismissal","chrismissal"),
		  "2116" => array("chris_patterson","chrispatterson"),
		  "2135" => array("agilecruz","christaylor"),
		  "2129" => array("colinjack","colinjack"),
		  "2113" => array("colin_ramsay","colinramsay"),
		  "2146" => array("derekgreer","derekgreer"),
		  "2128" => array("derickbailey","derickbailey"),
		  "2138" => array("eric","ericanderson"),
		  "2120" => array("hex","erichexter"),
      "2115" => array("evanhoff","evanhoff"),
		  "2133" => array("gabrielschenker","gabrielschenker"),
		  "2127" => array("thatotherguy","gregorylong"),
		  "2139" => array("jagregory","jamesgregory"),
      "2108" => array("jasdeep_singh","jasdeepsingh"),
		  "2104" => array("jason_meridth","jasonmeridth"),
		  "2114" => array("jimmy_bogard","jimmybogard"),
		  "2107" => array("joe_ocampo","joeocampo"),
		  "2110" => array("joeydotnet","joeybeninghove"),
      "2144" => array("johnvpetersen","johnpeterson"),
		  "2122" => array("johnteague","johnteague"),
		  "2126" => array("joshuaflanagan","joshuaflanagan"),
		  "2103" => array("joshua_lockwood","joshualockwood"),
		  "2142" => array("dahlbyk","keithdahlby"),
		  "2136" => array("louissalin","louissalin"),
		  "2111" => array("marcus_bratton","marcusbratton"),
		  "2124" => array("mokhan","mokhan"),
      "2105" => array("nelsonmontalvo","nelsonmontalvo"),
		  "2119" => array("rhouston","rayhouston"),
      "2143" => array("rodpaddock","rodpaddock"),
		  "2140" => array("rssvihla","ryansvihla"),
		  "2134" => array("scottdensmore","scottdensmore"),
      "2145" => array("scottgillen.rspec","scottgillenwater"),
		  "2125" => array("scottcreynolds","scottreynolds"),
		  "2137" => array("seanbiefeld","seanbiefeld"),
		  "2112" => array("sean_chambers","seanchambers"),
		  "2141" => array("sharoncichelli","sharoncichelli"),
		  "2130" => array("stevedonie","stevedonie"));

		printf(__('fixing image links for author: %s'), $post_author);
    if (array_key_exists($post_author, $user_paths)) {
		  printf(__(' %s'), $user_paths[$post_author][1]);
      
      $post_content = preg_replace('/_5F00_/', '_', $post_content);
      $post_content = preg_replace('/_2D00_/', '-', $post_content);
      $post_content = preg_replace('/%5B/', '[', $post_content);
      $post_content = preg_replace('/%5D/', ']', $post_content);

      $pattern = '/http:\/\/www\.lostechies\.com\/cfs-file\.ashx\/__key\/CommunityServer\.Blogs\.Components\.WeblogFiles\/' . $user_paths[$post_author][0] . '/';
      $post_content = preg_replace($pattern, 'http://lostechies.com/' . $user_paths[$post_author][1] . '/files/2011/03', $post_content);

      $pattern = '/http:\/\/www\.lostechies\.com\/cfs-filesystemfile\.ashx\/__key\/CommunityServer\.Blogs\.Components\.WeblogFiles\/' . $user_paths[$post_author][0] . '/';
      $post_content = preg_replace($pattern, 'http://lostechies.com/' . $user_paths[$post_author][1] . '/files/2011/03', $post_content);

      $pattern = '/cfs-file\.ashx\/__key\/CommunityServer\.Blogs\.Components\.WeblogFiles\/' . $user_paths[$post_author][0] . '/';
      $post_content = preg_replace($pattern, 'http://lostechies.com/' . $user_paths[$post_author][1] . '/files/2011/03', $post_content);

      $pattern = '/http:\/\/www.lostechies.com\/blogs\/' . $user_paths[$post_author][0] . '\/WindowsLiveWriter\//';
      $post_content = preg_replace($pattern, 'http://lostechies.com/' . $user_paths[$post_author][1] . '/files/2011/03', $post_content);

      $pattern = '/cfs-file\.ashx\/__key\/CommunityServer\.Components\.UserFiles\/00.00.00.21.\d{2}/';
      $post_content = preg_replace($pattern, 'http://lostechies.com/' . $user_paths[$post_author][1] . '/files/2011/03', $post_content);
    }
	...
	function parse_blogml()
	{
		...
		for ($i = 0; $i < $countResult; $i++)
		{
			$categoryName = $this->xPath->getData($result[$i]);
...
<p><?php _e('If a new user is created by WordPress, the password will be set, by default, to "changeme". Quite suggestive, eh? ;)'); ?></p>
	...
	function process_posts() {
		...
		if (count($this->old_new_post_mapping) > 0)
		{
		  ...
		  printf(__('CSV file name: %s'), $output_filename);
    ...
    $post_content = $this->convert_image_paths($post_content, $post_author);
		...
		for ($cat_index = 0; $cat_index < $cat_count; $cat_index++) {
			$categoryIDs[$cat_index] = $wpdb->escape($this->xPath->getAttributes($cat_results[$cat_index], 'ref'));
		}
        ...
        foreach ($categoryIDs as $categoryID) {
          $node = $this->xPath->match("/blog/categories/category[@id='".  $categoryID . "']/title");          
          $categoryName = $wpdb->escape($this->xPath->getData($node[0]));
          $cat_ID = (int) $wpdb->get_var("SELECT term_id FROM $wpdb->terms WHERE name = '$categoryName'");
          if ($cat_ID == 0) {
